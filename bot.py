...
from config import ACTIVE_LEAGUE_LOOKBACK_DAYS, CHANNEL_ID, CHECK_INTERVAL_MINUTES
from config import LIVE_ANNOUNCE_ENABLED, LIVE_CHECK_ENABLED, MAX_MATCHES_PER_CHECK, POST_DELAY_SECONDS
from config import TELEGRAM_TOKEN, TIER1_LEAGUES
from formatter import build_preview_caption, build_result_caption
from image_generator import _item_image, generate_match_preview_image, generate_match_result_image
from opendota_discovery import get_league_matches, get_live_league_matches, get_match_snapshot, get_player_info, get_recent_pro_matches, get_team_info
...
async def _send_photo_with_retry(image_buf, caption: str):
    assert bot is not None
    for attempt in range(1, TELEGRAM_SEND_RETRIES + 1):
        try:
            if hasattr(image_buf, "seek"):
                image_buf.seek(0)
            ...
            logger.warning("Telegram flood control. Retry %s/%s in %ss.", attempt, TELEGRAM_SEND_RETRIES, delay)
            await asyncio.sleep(delay)
        except (TimedOut, NetworkError) as exc:
            if attempt >= TELEGRAM_SEND_RETRIES:
                raise
            delay = 2 * attempt
            logger.warning("Telegram send failed (%s). Retry %s/%s in %ss.", exc, attempt, TELEGRAM_SEND_RETRIES, delay)
            await asyncio.sleep(delay)
...
def _has_complete_draft(details: dict) -> bool:
...
def _has_enough_bans(details: dict) -> bool:
    return len(details.get("bans") or []) >= MIN_PREVIEW_BANS


def _snapshot_has_final_result(snapshot: dict) -> bool:
    if not snapshot:
        return False
...
def get_recent_configured_matches() -> list[dict]:
    ...
    matches = get_recent_pro_matches(set(TIER1_LEAGUES), take=120)
    normalized = []
...
def get_raw_configured_live_rows() -> list[dict]:
    live_matches = get_live_league_matches(set(TIER1_LEAGUES), take=120)
...
def _filter_current_live_matches(live_matches: list[dict]) -> list[dict]:
    now = int(time.time())
    filtered = []
    for match in live_matches:
...
def get_finished_tracked_matches(current_live_ids: set[int] | None = None) -> list[dict]:
    ...
    for match_id, match in tracked.items():
        snapshot = get_match_snapshot(match_id)
        if match_id in current_live_ids and not _snapshot_has_final_result(snapshot):
            continue
...
def get_recent_finished_safety_matches(current_live_ids: set[int] | None = None) -> list[dict]:
    ...
    for match in get_recent_pro_matches(set(TIER1_LEAGUES), take=40):
        ...
        snapshot = get_match_snapshot(match_id)
        if not _snapshot_has_final_result(snapshot):
            continue
...
def get_live_configured_matches() -> list[dict]:
    return _filter_current_live_matches(get_raw_configured_live_rows())


def _result_asset_gaps(details: dict) -> tuple[list[str], list[str], list[str]]:
    players = details.get("players") or []
...
def _log_result_asset_quality(match_id: int, details: dict) -> None:
    players = details.get("players") or []
    missing_neutral, missing_neutral_icon, missing_buff_icon = _result_asset_gaps(details)
    logger.info(
...
async def post_match(match_summary: dict) -> None:
    ...
    _log_result_asset_quality(match_id, details)
    missing_neutral, missing_neutral_icon, _ = _result_asset_gaps(details)
    ...
    try:
        ...
        await _send_photo_with_retry(image_buf, caption)
...
async def check_and_post() -> None:
    ...
    live_matches = _filter_current_live_matches(raw_live_rows or []) if raw_live_rows is not None else None
...
async def post_historical_preview_and_result(match_summary: dict) -> None:
    ...
    if preview_details and _has_complete_draft(preview_details) and _has_enough_bans(preview_details):
        try:
            preview_image = generate_match_preview_image(preview_details)
            preview_caption = build_preview_caption(preview_details)
            await _send_photo_with_retry(preview_image, preview_caption)
            logger.info("Posted historical preview for %s", match_summary["match_id"])
        except Exception as exc:
...
async def main(
    ...
    scheduler.start()
    logger.info("Scheduler started. Checking every %s minutes.", CHECK_INTERVAL_MINUTES)
